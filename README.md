...
samplerdisc extract-banks "./Proteus 1" ./out    # loose E-mu .ebl banks (no disc image)
...
`extract-banks` is for E-mu Emulator X / Proteus X libraries that ship as loose files rather than a disc image — an `.exb` bank definition beside a `SamplePool` of `.ebl` samples, as archive.org's Proteus banks do. There is no container or disc filesystem to read; the files are already in the clear, so this points the same verified `.EBL` decoder straight at the directory ([ADR-0042](docs/adr/0042-loose-sample-banks-are-a-source-not-a-container.md)). `batch` also picks up loose-bank trees it finds beside disc images.
...
106 disc images from six sources — five archive.org collections and one direct index. From archive.org: [retro-sample-cds](https://archive.org/details/retro-sample-cds) (27), [Best Service ProSamples](https://archive.org/details/best-service-pro-samples-vol.-12-dance-vocals-akai-1-cd) (29), the [E-MU EIIIX Sound Library](https://archive.org/details/vol.-3-orchestral) (15), [archive-oldschoolscds](https://archive.org/details/archive-oldschoolscds) (15) and the [E-mu E-IV Series Factory Library](https://archive.org/download/e-mu-eiv-series-factory-library) (9); plus [nnty.fun's 90s sample CDs](https://nnty.fun/downloads/other/90ssamplecds/) (11). Seventy-seven flat `.iso`/`.bin`, seventeen compressed `.mdx`, seven raw CD images, two `.nrg`, one `.mds`/`.mdf` pair, two audio CDs. The figures below are regenerated by `scripts/benchmark.py` over the disc collection — the loose E-mu `.ebl` bank trees the tool also converts are staged apart and covered in *The E-mu EBL banks* below — and re-anchor as discs are added.

| | |
|---|---|
| Discs converted | 102 of 106 |
| Samples | 150 640 |
| Stereo pairs rejoined | 19 504 |
| Audio CD tracks | 161 |
| Duplicate audio suppressed | 5 719 |
| Entries not the file their entry placed | 2 |
| Entries skipped (damage) | 30 |
| Time | 79 s |
...
|---|---:|---:|---:|---:|
| AKAI | 45 | 72 491 | 16 071 | 25 |
| E-mu `EMU3` | 31 | 47 468 | 7 | 0 |
| ISO 9660 | 15 | 12 662 | — | 0 |
| HFS (SampleCell) | 2 | 1 144 | — | 0 |
| Kurzweil `KMSI` | 2 | 10 483 | 2 085 | 7 |
| Roland `S770 MR25A` | 5 | 6 392 | 1 341 | 0 |
| Audio CD | 2 | 0 | — | 0 |

"Stereo pairs" counts files joined from an `-L`/`-R` pair by name. E-mu's seven are the only ones across those discs, and they are a different and much rarer thing than the 17 929 samples whose record declares two channels.

Every sampler-filesystem WAV was checked against the disc it came from — **81 of 81 discs match exactly**, comparing the decoded audio against the disc's own bytes: a mono record's PCM is a run of the payload, and a stereo record's two channel blocks are the payload with its written interleave undone. 126 351 payloads, zero mismatches. The ISO 9660 discs are not in that count; where one ships a sound as both AIFF and WAV — the thirteen ProSamples discs — the publisher's own WAV is the oracle for what the conversion should produce, a stronger check still (below). The two SampleCell discs are not either — their audio is byte-swapped on the way to WAV like any AIFF, so instead both forks are checked against `machfs`, an independent HFS reader, byte for byte (D32/D33). The two audio CDs are in neither count: their tracks are cut from a stream by a cue, so there is no run of bytes on the disc to compare a track against.

170 305 WAV files were written in all — the samples, the stereo joins and the audio CD tracks. None is unreadable and none is zero-length. **301 are silent for their whole length, and every one of them matches the disc exactly**: 267 are the blank `15G-KIT…Z` slots on `ProSamples vol.15`, 24 are unused slots on the `Ditto Drums` ESI-32 disc, six are on a Proteus library that ships `Dead Air` as a sample, two are on a Roland disc, and one each on `E-mu Classics` and `Vintage`. That is what the discs hold, not something the decoder did.

Sample rates run from 6 000 to 50 000 Hz across 1 761 distinct values. The odd ones are real — E-mu writes rates like 24 444 and 27 778, and AKAI uses 33 075 (¾ of 44 100) and 29 400 (⅔) to trade bandwidth for memory. They are carried through exactly as the disc states them and never rounded.

The 25 AKAI entries not written are two different faults. **Two are payloads that are not the file the directory placed there** — their header carries another file's id, valid flag or name. There were 104: a rip that loses a run of blocks *inside* a partition slides a tail of one volume's samples forward, so each entry ends up pointing at the audio that took its place, and it does not need a partition to go missing (`Best Service - Alpha Dance II` declares six partitions and holds all six, and still displaced 21 of `AC.DRUMLOOPS`'s 22 samples). D38 recovers 102 of the 104 by searching backward, in the container's own block size, for a header that restates the entry's own name and word count, floored at the partition ([ADR-0045](docs/adr/0045-a-displaced-sample-is-recovered-by-its-own-name-and-word-count.md), [#35](https://github.com/bmxcode/samplerdisc/issues/35)). The two that remain are named, not written: `AMG - Loop Soup`'s one record whose start block lands mid-sample, so no header exists earlier to find, and `AKAI.S3000.Sound.Library.3`'s `20  CHINA2-R`, whose only backward match is a same-named, same-sized file in the previous partition — its bytes, not this one's. Each is refused with a line naming every field that disagrees and the entry that placed it, rather than being written out under a name that is not its own ([ADR-0027](docs/adr/0027-a-payload-must-be-the-file-its-entry-placed.md)).
...
The four that do not convert are accounted for: one S-550 disc present as both `.iso` and `.nrg` — a different format from the S-7xx and not yet read ([ADR-0014](docs/adr/0014-one-backend-per-on-disc-format.md)) — and one audio CD with no cue sheet present as both `.mdx` and `.cdr`. The two Digidesign SampleCell discs now convert: their AIFF (D32) and their Sound Designer II files (D33, [ADR-0040](docs/adr/0040-sound-designer-ii-is-decoded-from-the-data-fork.md)), the SDII loop carried into the WAV `smpl` chunk from the `sdLL` resource against the Digidesign spec (D39, [ADR-0046](docs/adr/0046-the-sd2-loop-is-decoded-from-sdll.md)).

### The E-mu EBL banks

`Digital Sound Factory - E-MU Vintage Pro` is an ISO 9660 disc whose 1 061 files are E-mu Emulator X `.EBL` sample banks rather than plain audio. It read 0 WAV until this deliverable; it now converts all 1 061, each named from the sample's own header (`EP4MKIIL A0`) under its bank folder rather than from the meaningless ISO sequence (`Vintage ProSL001`), with loop points carried where the file has them ([#55](https://github.com/bmxcode/samplerdisc/issues/55), [ADR-0033](docs/adr/0033-ebl-is-converted-on-a-disc-and-verified-by-a-render.md)). The check is the render itself: mattetti's publisher-grade FLAC of the whole bank, against which all 1 007 uniquely-named samples decode to the same rate and PCM byte for byte — the [ADR-0024](docs/adr/0024-the-aiff-twin-is-converted-and-deduplicated.md) oracle, for EBL ([formats/emu-ebl.md](docs/formats/emu-ebl.md)).

Vintage Pro is all mono, but the format also stores **stereo** non-interleaved (`LLLL…RRRR`), and that now converts too: a stereo record's two equal blocks are interleaved into a stereo WAV, anchored from the end of the audio and verified byte-for-byte against the publisher's render on 636 files across four banks ([ADR-0041](docs/adr/0041-stereo-ebl-is-interleaved-and-verified-by-the-render.md)). And an `.EBL` no longer needs a disc: the **Proteus 1/2/3** libraries ship as loose `.exb` + `SamplePool/*.ebl` in the clear, and `extract-banks` converts them through the same decoder — 686 banks, 685 mono and one genuinely-stereo, none refused ([ADR-0042](docs/adr/0042-loose-sample-banks-are-a-source-not-a-container.md)).
...
- **Roland S-550 and Ensoniq filesystems.** `Roland LCD1.iso` opens `* ROLAND S-550 *`, which shares nothing with the S-7xx format ([ADR-0014](docs/adr/0014-one-backend-per-on-disc-format.md)); no Ensoniq disc is in hand at all. Neither has a second specimen to check a backend against. The container layer opens all of these, so `export-iso` gets you the sectors meanwhile. Each backend is a self-contained module ([ADR-0003](docs/adr/0003-brand-neutral-pluggable-backends.md)), so adding one touches nothing else. (Kurzweil `KMSI` now reads, from D28/D29.)
...
- **E-mu `E4P1` presets are not read.** E-IV bank *audio* is read — through the chained `E3S1` sample directory where a bank is flat ([ADR-0020](docs/adr/0020-read-e-iv-through-its-sample-directory.md)), and through the `E3S1` chunks inside the bank where it is a native `FORM/E4B0` IFF file ([ADR-0032](docs/adr/0032-read-the-eiv-form-e4b0-bank-and-its-embedded-samples.md)); D25 recovered the 170 banks of 788 that stored their samples that second way. What is left is the presets, which hold the key ranges and envelopes a WAV cannot — hundreds per disc, and turning one into a playable instrument is ConvertWithMoss's job. A handful of banks carry a `FORM` of presets or text and no sample chunk (the `Credits` banks, and their credits text is [#53](https://github.com/bmxcode/samplerdisc/issues/53)); they hold no audio and are noted as such.
