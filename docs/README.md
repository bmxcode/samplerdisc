...
- **Sound Designer II on SampleCell discs is listed, not read.** The two SampleCell libraries read as HFS behind an Apple Partition Map now (D32, [ADR-0039](adr/0039-samplecell-is-read-as-hfs-behind-an-apple-partition-map.md)), and their AIFF converts — but `sonic-images-v2`'s 24 `Sd2f` files keep their audio in the HFS *resource* fork, a different and harder format than the flat-PCM data-fork AIFF the backend reads. They are named in a `list` and skipped by extraction rather than written as noise; recovering them means a resource-fork reader, deferred against the one corner of one disc that carries them. HFS+ (`drSigWord` `H+`) is likewise refused at the MDB signature; none is in hand.
