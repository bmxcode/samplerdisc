...
- **65 records declare two channels and are not stereo.** They put `start_R` half a payload on and then close `end_L` somewhere else, and their halves measure as unrelated audio — on `protozoa` the second half of six trombone records is another bank's record, byte for byte. They come out mono, which is right, but *what they are* is only established for `protozoa`: on `eiiix-1`'s 40 and `eiiix-2`'s 6 the halves are demonstrably not one performance and nothing says where the second one came from ([formats/emu3.md](formats/emu3.md), [ADR-0026](adr/0026-the-record-declares-the-channel-count.md)).
