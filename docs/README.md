...
| D32 | Digidesign SampleCell libraries read as HFS behind an Apple Partition Map — the AIFF payload converts through the existing path, verified byte-for-byte against the `machfs` reader ([ADR-0039](adr/0039-samplecell-is-read-as-hfs-behind-an-apple-partition-map.md)) | done |
| D33 | Sound Designer II (`Sd2f`) files on a SampleCell disc convert to WAV — the audio is the data fork (big-endian PCM), the rate/width/channels the resource fork's `STR ` resources; the resource-fork premise D32 held is corrected ([ADR-0040](adr/0040-sound-designer-ii-is-decoded-from-the-data-fork.md)) | done |
| D34 | Stereo E-mu `.EBL` interleaved into a stereo WAV and verified byte-for-byte against the publisher's render, after the reader was generalised across banks — reversing D26's stereo refusal ([ADR-0041](adr/0041-stereo-ebl-is-interleaved-and-verified-by-the-render.md)) | done |
| D35 | Loose E-mu `.ebl` banks (Proteus 1/2/3, shipped as files in the clear) convert through a dedicated `extract-banks` source that runs the verified decoder — a directory of sample files is a source, not a container ([ADR-0042](adr/0042-loose-sample-banks-are-a-source-not-a-container.md)) | done |
| D36 | E-IV disc provenance read from the sample-free `Credits`/`E-mu Systems 96` text banks' `E4P1` name field — the credit line alone, never the preset — into a per-disc `Credits.txt` sidecar under `--metadata`; eight banks, 89 lines across five discs ([ADR-0043](adr/0043-eiv-credits-are-metadata-read-from-the-e4p1-name-field.md)) | done |
...
- **Kurzweil programs and keymaps are not read.** A `.KRZ` bank's sample audio now converts ([ADR-0036](adr/0036-the-krz-bank-is-read-as-objects-and-verified-against-mpc2emu.md)), but its program and keymap objects — the key ranges, layers and envelopes — are not turned into an instrument. They are kept: `--keep-originals` writes the whole `.krz`, which [ConvertWithMoss](https://github.com/git-moss/ConvertWithMoss) reads directly, and that is the right home for them ([ADR-0011](adr/0011-the-deliverable-is-daw-ready-wav.md)). One sample shape the format allows — a single object holding several *mono* samples at different root keys — does not occur on the two Gigapack discs, so the reader's handling of it (one WAV per loaded header) is exercised only synthetically.
...
- **Sound Designer II on SampleCell discs is read (D33).** `sonic-images-v2`'s 24 `Sd2f` files now convert to stereo 44.1 kHz WAV. The premise D32 deferred them on was wrong: the audio is the **data fork** (big-endian PCM, which the backend already read), not the resource fork — only the rate/width/channels are in the resource fork, in three `STR ` resources ([ADR-0040](adr/0040-sound-designer-ii-is-decoded-from-the-data-fork.md), [formats/hfs.md](formats/hfs.md)). Verified byte-for-byte against `machfs` on both forks, and by the data-fork byte-swap self-check. What stays deferred is the SDII **loop**: `sdLL` may carry one, but there is no oracle to verify an interpretation against, so it is left rather than guessed ([ADR-0025](adr/0025-the-loop-is-decoded-the-root-key-is-not.md)). HFS+ (`drSigWord` `H+`) is still refused at the MDB signature; none is in hand.
- **A loose stereo `.ebl` outside a rendered bank rests on borrowed evidence.** Stereo `.EBL` converts (D34, [ADR-0041](adr/0041-stereo-ebl-is-interleaved-and-verified-by-the-render.md)) and loose banks convert (D35, [ADR-0042](adr/0042-loose-sample-banks-are-a-source-not-a-container.md)), verified byte-for-byte against the publisher's render wherever one exists — 636 files on Dance 2000 and the entirely-stereo grands. The Proteus banks D35 reads have no render, and one of their 686 files (`Snare w/Verb 28K`) is stereo: it is classified and interleaved by the `V12`/end-anchored rules proven on 636 files across four other banks, but that specific interleave is not checked against its own oracle. A Proteus render would settle it ([formats/emu-ebl.md](formats/emu-ebl.md)).
- **The E-mu `.exb` bank definition is not read.** Vintage Pro's `.exb` — and the loose Proteus `.exb` beside each `SamplePool` — holds the preset key ranges and zone mappings the same way an E-IV preset does; it is listed and, like EXS24 and `E4P1` presets, left to [ConvertWithMoss](https://github.com/git-moss/ConvertWithMoss) rather than converted ([ADR-0011](adr/0011-the-deliverable-is-daw-ready-wav.md)).
