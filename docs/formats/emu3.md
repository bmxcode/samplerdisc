...
`eiv-analogia` is stated as it came out. 441 of its 449 records pass every structural gate but only 34 carry audio loud enough at both ends to score on the **forward** window, and those 34 show nothing. That is a lack of power rather than a refutation: a near-whole-extent loop has almost no audio after its end for a forward correlation to work on. The **period / pre-start window** below — the 256 frames before each of the loop start and the loop end, which exploit `x[t] ≈ x[t − P]` and survive on a loop running to the last frame — does have power on it, and it gives the disc its own per-record confirmation for the first time. Of the six loops `eiv-analogia` keeps after the whole-extent refusal, **four splice at their own period** at *r* ≈ +0.99–1.00 as a clear local maximum against off-period lags, where the forward window could say nothing: `Usnotthem Chorus`, `Trilling Sound`, `The Lost Chord 1`, `The Lost Chord 2`. `Conscience Call` scores flat at every lag — a "no loop" whose start (frame 13 061) sits just past D23's start-side slack, so the whole-extent refusal did not catch it — and `Got Scratch?` ends below the 15%-of-peak loudness gate and cannot be scored, the same near-silence signature that marks a "no loop". The instrument is calibrated on `eiv-studio`'s real, forward-confirmed loops, where the same metric marks the period a clear local maximum on 57% of the scorable population against a null instrument's near-zero. So four of analogia's six residual loops now rest on the disc's own evidence, and the two that do not are named rather than hidden ([ADR-0044](../adr/0044-the-eiv-residual-loop-is-confirmed-by-its-period.md), verified in `tests/test_discs.py`).
